...
// addMonthsToIsoDate/computeStatus live in utils/licenseStatus.js - pure
// functions with no DB dependency, so they can be unit tested (tests/utils/
// licenseStatus.test.js) without opening a connection. computeStatus is the
// JS mirror of statusCaseSql() above, used only by createLicense()/
// updateLicense() below - one rule expressed twice (SQL for reads, JS for
// these two writes) rather than the three separate copies the original had
// (statusCaseSql() for reads, plus a duplicated inline CASE in each of
// createLicense's INSERT and updateLicense's UPDATE). `today` must be a
// 'YYYY-MM-DD' string from getServerToday() above.
const { computeStatus } = require('../utils/licenseStatus');
