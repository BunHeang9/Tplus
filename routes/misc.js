...
// --------------------------------------------------------------
// SSD UPGRADE  -  /api/ssd-upgrades
// Per-person SSD replacement tracking, joined to employee + equipment
// so the frontend gets names rather than bare IDs.
// --------------------------------------------------------------
router.get('/ssd-upgrades', authenticate, async (req, res) => {
  try {
...
// --------------------------------------------------------------
// SSD PROCUREMENT  -  /api/ssd-procurement
// Buy / Not Buy decision per hardware model.
// --------------------------------------------------------------
router.get('/ssd-procurement', authenticate, async (req, res) => {
  try {
    const pool = await poolPromise;
    const result = await pool.request().query(`
...
// Software / hardware licences and their expiry dates.
// --------------------------------------------------------------
router.get('/licenses', authenticate, async (req, res) => {
  try {
    const pool = await poolPromise;
    const result = await pool.request().query(`
...
// --------------------------------------------------------------
// SERVER USAGE  -  /api/server-usage
// Server specs and utilisation, joined to the equipment + owner.
// --------------------------------------------------------------
router.get('/server-usage', authenticate, async (req, res) => {
  try {
    const pool = await poolPromise;
    const result = await pool.request().query(`
      SELECT
...
// --------------------------------------------------------------
// ANTIVIRUS  -  /api/antivirus
// Antivirus install records per device.
// --------------------------------------------------------------
router.get('/antivirus', authenticate, async (req, res) => {
  try {
    const pool = await poolPromise;
    const result = await pool.request().query(`
      SELECT
...
// --------------------------------------------------------------
// DEVICE REPLACEMENT  -  /api/replacements
// Full old-device -> new-device history for everyone.
// --------------------------------------------------------------
router.get('/replacements', authenticate, async (req, res) => {
  try {
    const pool = await poolPromise;
    const result = await pool.request().query(`
      SELECT
...
// --------------------------------------------------------------
// CLOUD RATE  -  /api/cloud-rates
// Pricing per resource type and year.
// --------------------------------------------------------------
router.get('/cloud-rates', authenticate, async (req, res) => {
  try {
    const pool = await poolPromise;
    const result = await pool.request().query(`
...
// --------------------------------------------------------------
// CLOUD USAGE  -  /api/cloud-usage
// Actual monthly consumption and cost.
// --------------------------------------------------------------
router.get('/cloud-usage', authenticate, async (req, res) => {
  try {
    const pool = await poolPromise;
    const result = await pool.request().query(`
