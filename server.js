...
app.get('/', (req, res) => {
  res.json({
    ...
    architecture: 'MVC - routes define URLs, controllers handle requests, models hold all SQL',
    ...
    endpoints: {
      auth: {
        ...
        me: 'GET /api/auth/me',
        signup: 'POST /api/auth/signup  (public) { username, password, full_name } - always creates a viewer, pending admin approval',
        register: 'POST /api/auth/register  (admin only) - create an account directly, any role',
      },
      users: {
        list: 'GET /api/users  (admin) - all accounts, with pending_approval count',
        one: 'GET /api/users/:id  (admin)',
        update: 'PUT /api/users/:id  (admin) - change role, name, or approve a signup with { "is_active": true }',
        resetPassword: 'POST /api/users/:id/reset-password  (admin) { new_password }',
      },
      employees: {
        search: 'GET /api/employees/search?name=Fongmoua  - person plus all their devices',
        ...
        update: 'PUT /api/employees/:id  (admin only) - also how you deactivate a leaver: { "is_active": false, "left_date": "2026-07-30" }',
        remove: 'DELETE /api/employees/:id  (admin only, refused if they own equipment or have loan history)',
      },
      equipment: {
        list: 'GET /api/equipment',
        filters: 'GET /api/equipment?category=Server&location=VTE&department=TIT&status=Operational&unowned=true&q=probook',
        categories: 'GET /api/equipment/categories',
        one: 'GET /api/equipment/:id',
        reassignOwner: 'PUT /api/equipment/:id/owner  (admin only)',
        unassign: 'POST /api/equipment/unassign  (admin only)  { equipment_id | equipment_ids | owner_id, status }',
        update: 'PUT /api/equipment/:id  (admin only) - edit any detail',
      },
      stock: {
        add: 'POST /api/stock/add  (admin only) - add one new device into stock',
        assign: 'POST /api/stock/assign  (admin only) - hand a stock device to an employee',
      ...
      departments: {
        ...
        create: 'POST /api/departments  (admin only)  { department_code, department_name }',
      ...
      categories: {
        ...
        create: 'POST /api/categories  (admin only)  { category_name, description }',
        update: 'PUT /api/categories/:id  (admin only)',
      },
      borrow: {
        available: 'GET /api/borrow/available?category=Monitor  - what can be borrowed right now',
        current: 'GET /api/borrow/current?overdue=true  - what is out on loan',
        history: 'GET /api/borrow/history?equipment_id=&borrower_id=&from=&to=  - all loans, out and returned',
        returns: 'GET /api/borrow/returns?from=&to=&borrower_id=&late_only=true  - returned loans only, with days_kept and was_late',
        byBorrower: 'GET /api/borrow/borrower/:id?open=true  - one person\'s loans',
        one: 'GET /api/borrow/:id',
        borrow: 'POST /api/borrow  (admin)  { equipment_id, borrower_id | full_name, borrow_date, expected_return_date }',
        returnByEquipment: 'POST /api/borrow/return  (admin)  { equipment_id, return_date, condition_on_return }',
        returnByBorrowId: 'POST /api/borrow/:id/return  (admin)',
      },
      lookup: {
        universalSearch: 'GET /api/search?q=anything  - searches employees AND equipment',
