...
function project(row, columns, customFields = [], customValues = {}) {
  if (!row) return null;
  ...
  for (const f of customFields) out[f.field_key] = customValues[f.field_key] ?? null;
  return out;
}

function headersFor(columns, customFields = []) {
  return [
...
async function getByView(req, res, next) {
  ...
  try {
    ...
    const customFields = await customFieldModel.findByCategory(category.category_id);
    // One query for the whole page rather than one per row.
    const customValues = customFields.length > 0
      ? await customFieldModel.getValuesForMany(rows.map((r) => r.equipment_id))
      : {};
    ...
    res.json({
      ...
      columns: headersFor(columns, customFields),
      count: rows.length,
      items: rows.map((r) =>
        project(r, columns, customFields, customValues[r.equipment_id] || {})),
    });
  } catch (err) {
    next(err);
  }
...
async function createInView(req, res, next) {
  ...
  try {
    const category = await viewColumnModel.findCategoryByViewKey(viewKey);
    if (!category) return res.status(404).json({ error: `No view called "${viewKey}"` });

    const columns = await viewColumnModel.findByCategory(category.category_id);
    const editable = columns.filter((c) => c.is_editable).map((c) => c.field_name);
    ...
    const ALWAYS_WRITABLE = ['status', 'location', 'remark',
                             'category', 'category_id', 'owner_id', 'department'];
    const rejected = Object.keys(req.body).filter(
      (k) => !editable.includes(k) && !customKeys.includes(k)
    ...
    if (!department_id && req.body.department) {
      ...
      if (!dept) {
        return res.status(400).json({ error: `Unknown department "${req.body.department}"` });
      }
...
async function updateInView(req, res, next) {
  ...
  try {
    ...
    const ALWAYS_WRITABLE = ['status', 'location', 'remark',
                             'owner_id', 'department', 'department_id'];
    const rejected = Object.keys(req.body).filter(
      (k) => !editable.includes(k) && !customKeys.includes(k)
        && !ALWAYS_WRITABLE.includes(k)
    );
    ...
    await equipmentModel.update(req.params.id, req.body);
